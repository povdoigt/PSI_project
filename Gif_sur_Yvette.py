...
theta_U += phi
max_idx = df_U['X trans Uranus'].dropna().shape[0]
...
def animate(i):
    ...
    point_S.set_data([df_U['X trans Uranus'][idx]], [df_U['Y trans Uranus'][idx]])
    line.set_data(df_U['X trans Uranus'][:idx + 1], df_U['Y trans Uranus'][:idx + 1])

    return point, point_T, point_S, line

ani = anm.FuncAnimation(fig, animate, repeat=False, frames=5870, interval=1)
ax.legend()
plt.show()
