...
omega_S = 180/258 # omega du satélite en degrés par jours 
...
def animate(i):
    ...
    theta_T += omega_terre
    theta_S += omega_S 
    idx = int(theta_S / 180 * len(df_U['X trans mars']))
    ...
    point_S.set_data([df_U['X trans mars'][int(idx)]],[df_U['Y trans mars'][int(idx)]])
    line.set_data([df_U['X trans mars'][:int(idx)]],[df_U['Y trans mars'][:int(idx)]])
    
    return point,point_T,point_S,line

ani = anm.FuncAnimation(fig, animate, repeat=True, frames=258, interval=10)
ax.grid(c = 'w')
ax.legend()
plt.show()
